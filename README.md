...
- final pure signal evaluation
- experiment-group comparison and backfill

## What It Supports

- ML signal artifact training and evaluation
- Qlib-style signal metrics for artifacts: `IC`, `ICIR`, `Rank IC`, `Rank ICIR`
...
- integrated Qlib-compatible `Alpha158` and `Alpha360` views through the same unified factor registry and builder
...
Each trained artifact stores signal validation metrics in `metadata.json`, including `ic`, `ic_std`, `icir`, `rank_ic`, `rank_ic_std`, and `rank_icir`.
...
The technical factor library now also includes standard proxies such as `momentum_63_21`, `stochastic_k_14`, `stochastic_d_14_3`, `williams_r_14`, `money_flow_index_14`, `atr_14_pct`, `parkinson_volatility_20`, `garman_klass_volatility_20`, `upper_shadow_pct`, and `lower_shadow_pct`, along with Qlib-compatible factor views such as `KMID`, `OPEN0`, `MA5`, `RSQR20`, and `VOLUME59`.

Qlib compatibility is exposed as named factor sets plus tags, not as separate source families:

- `get_qlib_alpha158_factor_names()` returns the strict default Alpha158 view
- `get_qlib_alpha360_factor_names()` returns the full Alpha360 raw-history view
- overlapping names such as `OPEN0` are implemented once and tagged for both views
...
- `run_ml_experiment.py` evaluates signal quality with Qlib-style `IC / Rank IC` metrics plus a final `signal_test` window.
